...
namespace AutoAdminLib.Infrastructure {
    public class SqlBuilder<TKeyType, T> where T : BaseEntity<TKeyType> {
        ...
        public SqlBuilder<TKeyType, T> AppendCondition(string condition) {
            if (!string.IsNullOrWhiteSpace(condition)) {
        ...
        public SqlBuilder<TKeyType, T> InsertMany(IEnumerable<T> entities)
        {
            ...
            foreach (var entity in entities)
            {
                ...
                PopulateForInsert(entity, ref values, firstEntity ? columns : null);
                firstEntity = false;
            }
            _stringBuilder.AppendLine($"INSERT INTO {_repository.TableName} ({columns}) VALUES {values}");
            return this;
        ...
        private void PopulateForInsert(T entity, ref StringBuilder values, StringBuilder columns = null)
        {
            var excludedProperties = new [] {"IsIdAutoGenerated"};
        ...
        public SqlBuilder<TKeyType, T> UpdateOne(T entity, string[] columns)
        {
        ...
        public void Update(T entity, string[] columns)
        {
            ...
            {
                if ( Array.IndexOf(excludedProperties, property.Name) != -1 || 
                     Array.IndexOf(columns, property.Name) == -1 )
                {
                    continue;
                }
